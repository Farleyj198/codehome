...
struct Vertex {
	...
	glm::vec2 Tcoordinates = glm::vec2(0, 0);
	glm::vec3 Tvectors = glm::vec3(1, 0, 0);
...
//	out_color = vec4(1.0, 1.0, 0.0, 1.0);
	...
	vec3 texN = vec3(texture(normalMap,interUV));
...
int main(int argc, char** argv) {
	...
	double radius = 0.4 * (distance);
	 ...
	 v0.Tcoordinates = glm::vec2(0.5, 0.5);
...
//	vertOnly.push_back(radius);
	...
	for (int i = 0; i < sides; i++) {
		...
		float u = ((x / radius) + 1.0) / 2.0;
		float v = ((y / radius) + 1.0) / 2.0;
		v1.Tcoordinates = glm::vec2 (u,v);
...
*/
	...
	glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, Tcoordinates));
	glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, Tvectors));
	...
	if (texData != NULL) {
		...
		if (nrComponents == 3) {
			...
			cout << "ERROR: Unknown format! " << nrComponents << endl;
			glfwTerminate();
			exit(1);
		}
	...
	GLint uniformtectureID = glGetUniformLocation(progID, "normalMap");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for(int c=0; c<2; c++){
			...
			glUniform1i(uniformtectureID, 0);
